...
// Для N2O по СП: 10-литровый баллон, 6 МПа ≈ 3000 л газа
...
export default function N2OResult({ values, rooms }) {
  ...
  // оценка количества баллонов на сутки для осн./втор.
  const cylindersPerDay = Math.ceil(totalLPerDay / N2O_CYLINDER_VOLUME_L) || 0;
  const cylTooltip = `${Math.round(totalLPerDay).toLocaleString("ru-RU")} ÷ ${N2O_CYLINDER_VOLUME_L} = ${cylindersPerDay} шт`;

  // таблица
  const columns = [
    { title: "Оборудование", dataIndex: "equipment", key: "equipment" },
    { title: "Основной источник", dataIndex: "main", key: "main", align: "center" },
    { title: "Вторичный источник", dataIndex: "secondary", key: "secondary", align: "center" },
  ];
...
<Text
  type="secondary"
  style={{ fontSize: 12, lineHeight: 1.6, display: "block", marginTop: 12 }}
>
  <b>Принятые допущения:</b>
  <br />• Расход закиси азота рассчитывается по формуле:&nbsp;
  Vₙ₂ₒ&nbsp;=&nbsp;Vₘ&nbsp;×&nbsp;N&nbsp;×&nbsp;K&nbsp;×&nbsp;t&nbsp;×&nbsp;60&nbsp;(л/сут);
  <br />• В расчётах используется медицинская закись азота (сжиженный газ)
  с давлением 6&nbsp;МПа;
  <br />• Один баллон N₂O&nbsp;10&nbsp;л&nbsp;/&nbsp;6&nbsp;МПа&nbsp;≈&nbsp;3000&nbsp;л&nbsp;газообразной
  закиси&nbsp;азота;
  <br />• Система снабжения включает две рампы баллонов&nbsp;— рабочую и резервную
  с&nbsp;автоматическим переключением при опорожнении рабочей;
  <br />• Расчёт ведётся без учёта возможных потерь при переключении и продувке
  системы.
</Text>
